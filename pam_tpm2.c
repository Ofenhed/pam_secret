...
static int tpm_function(const char *sudoUser, const char *exec, const char *argument) {
    if (sudoUser == NULL) {
        sudoUser = "tss";
    }
    const char *tpmArgs[] = {SUDO_BIN, "-u", sudoUser, "--", exec, argument, NULL};
...
PAM_EXTERN int pam_sm_authenticate( pam_handle_t *pamh, int flags,int argc, const char **argv ) {
    ...
    for (int i = 0; i < argc; ++i) {
        int separator = -1;
        if (sscanf(argv[i], "pcr_%u=%n", &tmpRegister, &separator) && separator != -1) {
            if (strcmp(argv[i] + separator, pUsername) == 0) {
                pcrRegister = tmpRegister;
            }
        } else if (sscanf(argv[i], "as_user=%n", &separator) == 0 && separator != -1) {
    ...
    if (tpm_function(sudoUser, "/usr/bin/tpm2_pcrreset", (char*)&outputBuf) == -1) {
        fprintf(stderr, "tpm2_pcrreset failed\n");
        return PAM_SUCCESS;
    }
    snprintf((char*)&outputBuf, ARR_LEN(outputBuf), "%u:sha256=%s", pcrRegister, hashOutput);
