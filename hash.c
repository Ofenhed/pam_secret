...
void hmac(hash_state_t *h, const unsigned char *secret, int secret_len, const unsigned char *msg, int msg_len) {
    ...
    HMAC(EVP_sha256(), secret, secret_len, msg, msg_len, h->hashBuffers[h->active_buf ^= 1], &hash_len);
...
int hash_init_memfd(int hash_fd, int secret_fd, const unsigned char *msg, int msg_len) {
    ...
    hash_state_t *h = mmap(NULL, sizeof(hash_state_t), PROT_READ | PROT_WRITE, MAP_SHARED, hash_fd, 0);
    if (h == MAP_FAILED) {
        return -1;
    }
    secret_state_t *secret = mmap(NULL, sizeof(secret_state_t), PROT_READ, MAP_SHARED, secret_fd, 0);
...
int finalize_hash(int hash_fd, int secret_fd, sha256_hash_t *result) {
    ...
    HMAC(EVP_sha256(), secret, sizeof(secret_state_t), h->hashBuffers[h->active_buf], sizeof(sha256_hash_t), *result, &len);
